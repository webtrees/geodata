...
class ExportCommand extends AbstractBaseCommand
{
     ...
     * @param string     $prefix
     */
    private function exportData(Filesystem $source, Filesystem $destination, string $language, string $prefix)
    {
        ...
        foreach ($geojson_objects as $geojson_object) {
            $geojson = json_decode($source->read($geojson_object['path']));
     ...
     * @return string
     */
    private function createPngFromImage(string $image, int $width, int $height): string
    {
